...
from robosuite.models.objects import RoundNutObject, SquareNutObject, HookFrame, RatchetingWrenchObject, StandWithMount, StandWithMountHook
...
class NutAssemblyHangInsert(SingleArmEnv):
    """
    This class corresponds to the nut assembly task for a single robot arm. Includes hook to hang peg.
    ...
    def _load_model(self):
        """
        Loads an XML model, puts it in self.model, and initializes the nuts and tools.
        """
        super()._load_model()
        ...
        # Define and create stand, frame, and tool with their specific parameters
        self.stand_args = dict(
            ...
            size=(
                ...
                (14.0 / 100.0),
                (16.0 / 100.0),
            ),  # 14 cm x 12 cm base, with 16 cm height
            mount_location=(0.0, (4.5 / 100.0)),
            mount_width=0.05,
            wall_thickness=0.01,
            hook_length=0.1,
            hook_height=0.1,
        ...
        self.stand = StandWithMountHook(**self.stand_args)

        self.frame_args = dict(
            name="frame",
            frame_length=0.095,
            frame_height=0.18,
            frame_thickness=0.0075,
            hook_height=0.012,
            grip_location=0.0455,
            grip_size=(0.0127, 0.03175),
            tip_size=(0.0127, 0.002, 0.0065, 0.01905),
            density=500.0,
            solref=(0.02, 1.0),
            solimp=(0.998, 0.998, 0.001),
        )

        # Add the hook to the placement initializer with specific samplers
        objects = [self.stand]
        ...
        # Adding each tool, frame, and stand object with its specific placement configuration
        ...
        # Task includes arena, robot, and all objects of interest
        self.model = ManipulationTask(
            mujoco_arena=mujoco_arena,
            mujoco_robots=[robot.robot_model for robot in self.robots],
            mujoco_objects=[*self.nuts, self.stand],
        )
    ...
    def _create_nut_sensors(self, nut_name, modality="object"):
        ...
        def nut_to_eef_pos(obs_cache):
            ...
            obs_cache[f"{nut_name}to{pf}eef_quat"] = rel_quat
            return rel_pos

        @sensor(modality=modality)
        def nut_to_eef_quat(obs_cache):
            return (
                obs_cache[f"{nut_name}to{pf}eef_quat"] if f"{nut_name}to{pf}eef_quat" in obs_cache else np.zeros(4)
            )

        sensors = [nut_pos, nut_quat, nut_to_eef_pos, nut_to_eef_quat]
        names = [f"{nut_name}_pos", f"{nut_name}_quat", f"{nut_name}to{pf}eef_pos", f"{nut_name}to{pf}eef_quat"]
...
class NutAssemblySingle(NutAssemblyHangInsert):
    """
    ...
    def _init_(self, **kwargs):
        ...
        super()._init_(single_object_mode=1, **kwargs)


class NutAssemblySquare(NutAssemblyHangInsert):
    """
    ...
    def _init_(self, **kwargs):
        ...
        super()._init_(single_object_mode=2, nut_type="square", **kwargs)


class NutAssemblyRound(NutAssemblyHangInsert):
    """
    ...
    def _init_(self, **kwargs):
        assert "single_object_mode" not in kwargs and "nut_type" not in kwargs, "invalid set of arguments"
        super()._init_(single_object_mode=2, nut_type="round", **kwargs)