...
class StandWithMountHook(CompositeObject):
    """
    ...
    Args:
        ...
        hook_length (float): Length of the horizontal hook arm
        hook_thickness (float): Thickness of the hook arm
    ...
    def _get_geom_attrs(self):
        ...
        add_to_dict(
            dic=obj_args,
            geom_types="box",
            geom_locations=(0, 0, self.base_thickness + (self.size[2] - self.base_thickness) / 2),
            geom_quats=(1, 0, 0, 0),
            geom_sizes=(self.hook_length / 2, self.hook_thickness / 2, self.hook_thickness / 2),
            geom_names=self._hook_geom,
            geom_rgbas=None if self.use_texture else self.rgba,
            geom_materials=self.mat_name if self.use_texture else None,
            geom_frictions=self.friction,
        )
